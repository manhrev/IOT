...
// QueryFeeds queries the feeds edge of a Group.
func (c *GroupClient) QueryFeeds(gr *Group) *FeedQuery {
	query := &FeedQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := gr.ID
		step := sqlgraph.NewStep(
			...
			sqlgraph.To(feed.Table, feed.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, group.FeedsTable, group.FeedsPrimaryKey...),
		)
