...
func (fq *FeedQuery) QueryGroup() *GroupQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, feed.GroupTable, feed.GroupPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(fq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (fq *FeedQuery) sqlAll(ctx context.Context) ([]*Feed, error) {
	...
	if query := fq.withData; query != nil {
		...
		for _, n := range neighbors {
			fk := n.feed_data
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "feed_data" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "feed_data" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := fq.withGroup; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(feed.GroupPrimaryKey[1], fks...))
			},
