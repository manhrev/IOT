...
func (dq *DataQuery) GroupBy(field string, fields ...string) *DataGroupBy {
	grbuild := &DataGroupBy{config: dq.config}
...
func (dq *DataQuery) Select(fields ...string) *DataSelect {
	...
	selbuild := &DataSelect{DataQuery: dq}
	selbuild.label = data.Label
...
func (dq *DataQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Data, error) {
	...
	if query := dq.withFeed; query != nil {
		if err := dq.loadFeed(ctx, query, nodes, nil,
			func(n *Data, e *Feed) { n.Edges.Feed = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DataQuery) loadFeed(ctx context.Context, query *FeedQuery, nodes []*Data, init func(*Data), assign func(*Data, *Feed)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "feed_data" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dgb *DataGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ds *DataSelect) sqlScan(ctx context.Context, v any) error {
